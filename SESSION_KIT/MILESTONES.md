...
## M0 — Setup  `status: not started`
- [ ] Preflight pass/fail table reported (Node 22, git, gh; adb/blender/
      godot optional)
- [ ] Scaffold: `npm create @iwsdk@latest kubborama -- --yes --target vr
      --language ts --install --git`
- [ ] iwsdk.config.json: `physics: true` (CRITICAL — default is false!),
      locomotion off
- [ ] Emulator scene runs; MCP verified (screenshot + simulated input)
- [ ] TS strict + eslint + prettier + vitest wired; CI: typecheck+lint+
      test+build on push
- [ ] GitHub Pages deploy via Actions (base './' — verify before changing)
- [ ] Deployed URL verified on desktop AND Quest browser
- [ ] GitHub labels created (bug/feature/tech-debt/follow-up)
- [ ] Secret scanning + push protection enabled on the repo (public!);
      .gitignore covers .env and assets/raw/
- [ ] Docs decomposed: CLAUDE.md merged (from SESSION_KIT/
...
- [ ] README with headset testing steps (dynamic port via dev status)
