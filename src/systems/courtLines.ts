 ...
 * a boolean write is cheap, but there's no reason to touch four
 * entities every frame when the value hasn't changed.
 */
export class CourtLinesSystem extends createSystem({
  ...
  // (also false) — the four line nodes are already hidden at load, so
  // this only needs to act on an actual change from here.
